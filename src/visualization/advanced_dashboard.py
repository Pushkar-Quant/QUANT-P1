...
# Page configuration
st.set_page_config(
    page_title="Adaptive Liquidity Provision Engine - Market Making Analytics",
    page_icon="📊",
    layout="wide",
    initial_sidebar_state="expanded",
    menu_items={
        'Get Help': 'https://github.com/YOUR_USERNAME/adaptive-liquidity-provision',
        'Report a bug': 'https://github.com/YOUR_USERNAME/adaptive-liquidity-provision/issues',
        'About': '''
        # Adaptive Liquidity Provision Engine
        
        A production-ready reinforcement learning framework for market making.
        ...
        if run_sim:
            try:
                with st.spinner("🔄 Running simulation (20-30 seconds)..."):
                    ...
                    # Create OrderFlowConfig with volatility parameter
                    from src.simulation.order_flow import OrderFlowConfig
                    
                    order_flow_config = OrderFlowConfig(
                        base_volatility=volatility  # Use volatility from slider
                    )
                    
                    sim_config = SimulationConfig(
                        initial_midprice=initial_price,
                        tick_size=0.01,
                        order_flow_config=order_flow_config
                    )
                    ...
                    if len(result.pnl_series) == 0:
                        st.error("⚠️ Simulation produced no data. This might be a configuration issue.")
                        st.info("💡 Try: Refresh page and run again with default parameters.")
                    elif result.metrics.total_pnl == 0 and len(result.pnl_series) > 1:
                        st.warning("⚠️ Simulation completed but P&L is zero. The strategy may not have executed any trades.")
                        st.info(f"📊 Simulation ran for {len(result.pnl_series)} steps. Check parameters and try again.")
                    ...
                    with st.expander("📊 Simulation Details"):
                        ...
                        if result.metrics.total_trades == 0:
                            st.warning("⚠️ No trades executed. Strategy may be too passive or market conditions unsuitable.")
                    
            except Exception as e:
                st.error(f"❌ Error during simulation: {str(e)}")
                st.info("💡 Try refreshing the page and running again with default parameters.")
                import traceback
